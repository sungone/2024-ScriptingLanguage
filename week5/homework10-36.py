...
class MainGUI :
    def next(self) :
        ...
        self.canvas.create_rectangle(10 + self.current * barWidth , height - (height - 10) * self.counts[self.current] / self.maxCount , 10 + (self.current + 1) * barWidth , height - 10 , fill = 'red' , tags='current_bar')
        if key == self.counts[self.current] :
            tkinter.simpledialog.messagebox.showinfo("찾았다" , "{0}".format(key))
        else :
            self.current += 1
    def reGen(self) :
        ...
        for i in range(20) :
            self.canvas.create_rectangle(10 + i * barWidth , height - (height - 10) * self.counts[i] / self.maxCount , 10 + (i + 1) * barWidth , height - 10 , tags='histogram')
